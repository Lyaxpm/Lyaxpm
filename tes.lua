...
local Window = Rayfield:CreateWindow({
    Name = "0xGen Fish It",
    LoadingTitle = "0xGen - Fish It",
    LoadingSubtitle = "DEVELOPER BY Rud Az",
...
local Config = {
    ...
    -- WhatsApp Hooked Config
    Hooked = {
        Enabled = false,
        APIUrl = "http://103.176.79.203:5001/send/message",
        PhoneNumber = "",
        TargetRarities = {}
    }
}
...
local SaveFileName = "0xGenFishItSettings_" .. LocalPlayer.UserId .. ".json"
...
-- ===== WHATSAPP HOOKED NOTIFICATION SYSTEM =====
local Hooked = {}

function Hooked:SendWhatsAppMessage(fishInfo)
    if not Config.Hooked.Enabled then
        print("[🔔 WHATSAPP] Notifications disabled in config")
        return
    end
    
    if Config.Hooked.PhoneNumber == "" then
        warn("[❌ WHATSAPP] Phone number not configured!")
        return
    end
    
    -- CRITICAL FIX: Ensure tier exists
    if not fishInfo.Tier then
        warn("[❌ WHATSAPP] Fish info missing tier data!")
        return
    end
    ...
    print(string.format("[🔔 WHATSAPP] Processing: %s | Tier: %d | Rarity: %s", 
        fishInfo.Name or "Unknown", fishInfo.Tier, fishRarity))
    
    -- Check rarity target dengan case-insensitive comparison
    if #Config.Hooked.TargetRarities > 0 then
        ...
        if not shouldSend then
            print(string.format("[❌ WHATSAPP] Skipped - %s not in target rarities", fishRarity))
            return
        end
    end
    
    print("[✅ WHATSAPP] Sending notification for " .. (fishInfo.Name or "Unknown"))
    
    -- Format message
    local message = self:FormatWhatsAppMessage(fishInfo)
    
local success, result = pcall(function()
    local dataToSend = {
        phone = Config.Hooked.PhoneNumber .. "@s.whatsapp.net",
        message = message,
        is_forwarded = false
    }
    
    local jsonBody = HttpService:JSONEncode(dataToSend)
    
    local requestHeaders = {
        ["Content-Type"] = "application/json", 
        ["Authorization"] = "Basic Zzpn"
    }

    local function sendRequest(requestMethod)
        local response = requestMethod({
            Url = Config.Hooked.APIUrl,
            Method = "POST",
        ...
        if response.Success then
            ...
            if data and data.code == "SUCCESS" then
                return true
            else
                return false, data.message or "Response code was not SUCCESS"
            end
        else
            return false, "HTTP request failed (Status: " .. tostring(response.StatusCode or "N/A") .. ")"
        end
    end

    if http_request then
        return sendRequest(http_request)
    elseif syn and syn.request then
        return sendRequest(syn.request)
    else
...
if success then
    ...
    warn("[❌ WHATSAPP] Failed to send: " .. tostring(result))
end
end

function Hooked:FormatWhatsAppMessage(fishInfo)
    ...
    -- Build clean message
    local message = "🎣 *0xGen FISH IT V1*\n"
    message = message .. "💻 *DEVELOPER: Rud Az*\n\n"
    ...
    message = message .. "   TIKTOK: @rud.az_\n"
    message = message .. "   INSTAGRAM: @rud.az_\n"
    message = message .. "   ROBLOX: @GenAvatarAgin\n\n"
...
if FishCaught then
    FishCaught.OnClientEvent:Connect(function(data)
        ...
        Hooked:SendWhatsAppMessage(fishInfo)
    end)
    
    print("[✅] Fish catch listener initialized")
else
    warn("[❌] FishCaught remote not found! WhatsApp notifications will not work.")
end

-- ===== AUTO REJOIN SYSTEM =====
local RejoinSaveFile = "0xGenRejoinData_" .. LocalPlayer.UserId .. ".json"
...
local function CreateUI()
    ...
    -- ===== WHATSAPP HOOKED TAB =====
    local Tab9 = Window:CreateTab("🔔 WhatsApp Hooked", 4483362458)
    
    Tab9:CreateSection("WhatsApp Hooked Configuration")
    
    Tab9:CreateToggle({
        Name = "Enable WhatsApp Hooked",
    ...
    Tab9:CreateInput({
        Name = "Phone Number",
        PlaceholderText = "628xxxxxx (without +)",
        RemoveTextAfterFocusLost = false,
        CurrentValue = Config.Hooked.PhoneNumber,
        Callback = function(Value)
            Config.Hooked.PhoneNumber = Value
            SaveSettings()
        end
    })
    
    Tab9:CreateLabel("Format: 628xxxxx (tanpa +)")
    ...
    Tab9:CreateButton({
        Name = "Test WhatsApp Notification",
        Callback = function()
            local testFish = {
            ...
            Hooked:SendWhatsAppMessage(testFish)
            Rayfield:Notify({
                Title = "WhatsApp Test",
    ...
    Tab9:CreateLabel("WhatsApp Hooked will send notifications when you catch fish with selected rarities.")
    Tab9:CreateLabel("Make sure your WhatsApp number is registered with the API service.")
    ...
    Tab11:CreateParagraph({
        Title = "🆕 WhatsApp Hooked",
        Content = "• Send fish catch notifications to WhatsApp\n• Filter by rarity\n• Beautiful formatted messages\n• Real-time fishing statistics"
    })
